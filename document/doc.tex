...
\begin{equation}
    y_1 = \frac{x_1-x_2}{\sqrt{\epsilon}} \, , \hspace{1cm} y_2 = x_1 + x_2 \quad .
\end{equation}
...
independent of $\model$ and $\nuisance$ once we have chosen the form of the generative model.
...
\begin{equation}
    ...
    \pr{\model|\data}
    \, \phi (\model) \, \dd\model
    \,\approx\, \frac{1}{M} \sum_{i=1} ^M \phi(\model_i) \quad.
\end{equation}
...
\FOR{$i \in \{0, 1\}$}
    \FOR{$k = 1, \ldots, K/2$} \label{line:parallelloop}
        \STATE {\footnotesize\it // This loop can now be done in parallel for all $k$}
...
spectral methods have been used extensively \citep[for
  example][]{Dunkley:2005}. Below, we describe one---an
affine-invariant measure of performance---the autocorrelation time.
...
affine-invariant measure of performance, and it doesn't have an a
...
evaluated with small amounts of data (that is, in short chains).
...
In particular, we have found that increasing the number of walkers
improves the acceptance fraction of any chain where it falls too low.
