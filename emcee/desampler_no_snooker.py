...
class DESampler(Sampler):
    ...
    **Warning**: The :attr:`chain` member of this object has the shape:
    ...
    def __init__(self, nwalkers, dim, lnpostfn, gamma=None, sigma_gamma=1e-2, autoscale_gamma=True, norm_width=1e-4, args=[], kwargs={},
        ...
        self.prev_iteration_acceptance_fraction = 0.
        ...
        * ``blobs`` - (optional) The metadata "blobs" associated with the
        ...
        for i in range(int(iterations)):
            ...
            if np.any(acc):
                ...
                self.prev_iteration_acceptance_fraction = np.sum(acc)/len(acc)
    ...
    def _propose_stretch(self, p0, lnprob0):
        ...
        * ``blob`` - The new meta data blobs or ``None`` if nothing was
        ...
        if self.iterations == 0:
            ...
            #print(self.prev_iteration_acceptance_fraction)
            #print(self.gamma)
            if self.autoscale_gamma:
                if self.prev_iteration_acceptance_fraction < 0.2:
                    self.gamma *= 0.9
                elif self.prev_iteration_acceptance_fraction > 0.31:
                    self.gamma *= 1.1
                else:
                    self.gamma *= (self.prev_iteration_acceptance_fraction/0.25)**0.5
            else:
                self.gamma = 2.38 / (2*self.dim)**0.5
            #print(self.gamma)
