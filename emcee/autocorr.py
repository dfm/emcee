...
def function(x, axis=0, fast=False):
    ...
    x = np.atleast_1d(x)
    if len(x.shape) <= 1:
        return function_1d(x)
    xprime = np.moveaxis(x, axis, -1)
    return np.array([function_1d(row) for row in xprime])
...
def integrated_time(x, low=10, high=None, step=1, c=5, full_output=False,
                    axis=0, fast=False, tol=50, quiet=False):
