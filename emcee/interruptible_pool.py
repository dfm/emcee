...
import multiprocessing.pool, signal, functools


def _initializer_wrapper (actual_initializer, *rest):
    """We ignore SIGINT. It's up to our parent to kill us in the typical condition
    of this arising from ^C on a terminal. If someone is manually killing us
    with that signal, well, ... nothing will happen.
...
class InterruptiblePool (multiprocessing.pool.Pool):
    ...
    def __init__ (self, processes=None, initializer=None, initargs=(), **kwargs):
        ...
        super (InterruptiblePool, self).__init__ (processes, new_initializer, initargs, **kwargs)


    def map (self, func, iterable, chunksize=None):
        '''
        Equivalent of `map()` builtin, without swallowing KeyboardInterrupt.
