...
class Tests:
    def setUp(self):
        ...
        self.cov = 0.5 - np.random.rand(self.ndim ** 2).reshape(
            (self.ndim, self.ndim)
        )
        ...
        self.p0 = [
            0.1 * np.random.randn(self.ndim) for i in range(self.nwalkers)
        ]
    ...
    def check_sampler(self, N=None, p0=None):
        ...
        assert np.all(
            (np.mean(chain, axis=0) - self.mean) ** 2 / self.N ** 2 < maxdiff
        )
        assert np.all(
            (np.cov(chain, rowvar=0) - self.cov) ** 2 / self.N ** 2 < maxdiff
        )
    ...
    def test_mh(self):
        self.sampler = MHSampler(
            self.cov, self.ndim, lnprob_gaussian, args=[self.icov]
        )
    ...
    def test_deterministic(self):
        ...
        assert np.all(p1 == p3) and np.all(
            lnprob1 == lnprob3
