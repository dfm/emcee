...
class EnsembleSampler(MCSampler):
	...
	def run_mcmc(self,position,randomstate,iterations,lnprobinit=None):
		...
		"""
		for pos,prob,state in self.sample(position,lnprobinit,randomstate,
	...
	def sample(self,position0,lnprob,randomstate,*args,**kwargs):
		...
		for k in xrange(iterations):
			...
			newposition = position[rint] + zz[:,np.newaxis]*(position-position[rint])
			...
			# to the chain
			## NOTE -- this is creating a new array -- (iterations)^2 scaling
